...
class Esp8266 : public Module<>
{
 ...
 private:
  ...
  bool IsConnected()
  {
    ...
    static constexpr int kStateNeverConnected = 0;
    // ESP station has connected to an AP, but does not get an IPv4 address yet.
    static constexpr int kStateConnectedNoIP = 1;
    // ESP station has connected to an AP, and got an IPv4 address.
    static constexpr int kStateConnectedWithIP = 2;
    // ESP station is in Wi-Fi connecting or reconnecting state.
    static constexpr int kStateConnecting = 3;
    // ESP station is in Wi-Fi disconnected state.
    static constexpr int kStateDisconnected = 4;
    ...
    // Substract 1 from the response length to the extra formatting character in
